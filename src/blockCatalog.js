...
export const BLOCK_CATALOG = [
  ...
  // For every oembed-type block (youtube/vimeo/…): supply only `url`.
  // The public API resolves the embed payload server-side via
  // oembed.eno.do and fills `oembed` before storage. An invalid URL or
  // an oembed lookup failure rejects the whole create/update with 400.
  {
    type: 'youtube',
    description: 'YouTube embed. Send only the URL; the API fills the oembed payload.',
    shape: '{ type: "youtube", data: { url: string } }',
    example: { type: 'youtube', data: { url: 'https://www.youtube.com/watch?v=dQw4w9WgXcQ' } },
  },
  {
    type: 'dailymotion',
    description: 'Dailymotion embed. Send only the URL; the API fills the oembed payload.',
    shape: '{ type: "dailymotion", data: { url: string } }',
    example: { type: 'dailymotion', data: { url: 'https://www.dailymotion.com/video/x7tgad0' } },
  },
  {
    type: 'vimeo',
    description: 'Vimeo embed. Send only the URL; the API fills the oembed payload.',
    shape: '{ type: "vimeo", data: { url: string } }',
    example: { type: 'vimeo', data: { url: 'https://vimeo.com/123' } },
  },
  {
    type: 'x',
    description: 'X / Twitter embed. Send only the URL; the API fills the oembed payload.',
    shape: '{ type: "x", data: { url: string } }',
    example: { type: 'x', data: { url: 'https://x.com/handle/status/123' } },
  },
  {
    type: 'tiktok',
    description: 'TikTok embed. Send only the URL; the API fills the oembed payload.',
    shape: '{ type: "tiktok", data: { url: string } }',
    example: { type: 'tiktok', data: { url: 'https://www.tiktok.com/@user/video/123' } },
  },
  {
    type: 'facebook',
    description: 'Facebook embed (post or video). Send only the URL; the API fills the oembed payload.',
    shape: '{ type: "facebook", data: { url: string } }',
    example: { type: 'facebook', data: { url: 'https://www.facebook.com/user/posts/123' } },
  },
  {
    type: 'instagram',
    description: 'Instagram embed. Send only the URL; the API fills the oembed payload.',
    shape: '{ type: "instagram", data: { url: string } }',
    example: { type: 'instagram', data: { url: 'https://www.instagram.com/p/ABCDEF/' } },
  },
  {
    type: 'table',
